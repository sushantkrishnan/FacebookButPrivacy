...
{
	mysqli_connect("localhost","sushant","DBMSproject");
	...
	if($count1>0)
	{
		...
		$que2=mysqli_query("select * from user_profile_pic where user_id=$userid");
		$count2=mysqli_num_rows($que2);

		if($count2>0)
		{
			$que3=mysqli_query("select * from user_secret_quotes where user_id=$userid");
			$count3=mysqli_num_rows($que3);
			if($count3>0)
			{
				$que4=mysqli_query("select * from user_secret_quotes where user_id=$userid");
				...
				else
				{
					...
					$query1=mysqli_query("select * from users where Email='$user'");
					$rec1=mysqli_fetch_array($query1);
					$userid=$rec1[0];
					mysqli_query("update user_status set status='Online' where user_id='$userid'");
	...
	{
		$que5=mysqli_query("select * from users where Email='$user'");
